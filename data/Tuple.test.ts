...
function testNodeFindWhere(size: number, type: Type, detail: Detail) {
  describe(`when finding a node by id in a graph of ${size} nodes`, () => {
    ...
    beforeEach(() => {
      ...
      [key, value] = 
        detail === "small" ? pick_random_key_value_tuple_small(random_node) : pick_random_key_value_tuple_large(random_node)

      const filterTuplesByKeyValue = detail === "small" ? filterSmallTupleByKeyValue : filterLargeTupleByKeyValue;
...
function testNodeUpdateWhere(size: number, type: Type, detail: Detail) {
  describe(`when updating nodes on the condition KEY=VALUE in a graph of ${size} nodes`, () => {
    ...
    beforeEach(() => {
      ...
      _large_tuple_callback = node => filterTuplesByKeyValue(node, key, value) ? increment_tuple_x_coordinate(node) : node;
    });
...
function testNodeDeleteWhere(size: number, type: Type, detail: Detail) {
  describe(`when deleting nodes in a graph of ${size} nodes on the condition where KEY=VALUE `, () => {
    ...
    beforeEach(() => {
      ...
      first_node = nodes[0];
      value = detail === "small" ? node[4] : node[5][1] 
      key = "icon";
      random_node = nodes[0];


      const filterTuplesByKeyValue: Function = detail === "small" ? filterSmallTupleByKeyValue : filterLargeTupleByKeyValue;
      expected_remaining_nodes = nodes.filter(node => !filterTuplesByKeyValue(node, key, value));
    });
    it(`then all the nodes matching the condition should be removed, taking up a specific amount of time`, () => {
      // Contains nodes where they condition K=V failed
      let deleted_nodes = Benchmark.Performance(() =>
